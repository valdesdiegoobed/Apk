...
private fun App() {
    ...
    when (screen) {
        ...
        Screen.CLIENT_DETAIL -> {
            ...
            if (client == null) screen = Screen.CLIENTS else ClientDetailScreen(
                client = client,
        ...
        Screen.BROWSER -> BrowserScreen(client = selectedClient(), onBack = { screen = Screen.CLIENT_DETAIL })
    }
}
...
private fun ClientsScreen(clients: List<Client>, onNew: () -> Unit, onOpen: (String) -> Unit) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Expediente Clientes") }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
            OutlinedTextField(query, { query = it }, Modifier.fillMaxWidth(), label = { Text("Buscar por nombre, CURP o RFC") }, singleLine = true)
            Spacer(Modifier.height(12.dp))
            Button(onClick = onNew, modifier = Modifier.fillMaxWidth()) { Text("Nuevo cliente") }
            Spacer(Modifier.height(16.dp))
            if (filtered.isEmpty()) Card(Modifier.fillMaxWidth()) {
                Text(if (clients.isEmpty()) "Aún no hay clientes. Pulsa Nuevo cliente." else "No se encontraron resultados.", Modifier.padding(16.dp))
...
private fun ClientFormScreen(existing: Client?, onBack: () -> Unit, onSave: (Client) -> Unit) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(if (existing == null) "Nuevo cliente" else "Editar cliente") }) }) { padding ->
        LazyColumn(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            item { Button(onClick = {
                if (name.isBlank()) error = "El nombre es obligatorio." else onSave(Client(
                    id = existing?.id ?: UUID.randomUUID().toString(), name = name.trim(), curp = curp.trim(), rfc = rfc.trim(),
                    nss = nss.trim(), phone = phone.trim(), email = email.trim(), afore = afore.trim(), notes = notes.trim(),
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ClientDetailScreen(client: Client, onBack: () -> Unit, onEdit: () -> Unit, onOpenBrowser: () -> Unit, onUpdate: (Client) -> Unit, onDelete: () -> Unit) {
    val context = LocalContext.current
    ...
    val picker = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        if (uri != null) {
            ...
            onUpdate(client.copy(documents = client.documents + ClientDocument(label = pendingLabel, uri = uri.toString(), mimeType = context.contentResolver.getType(uri).orEmpty())))
        }
    }
    Scaffold(topBar = { TopAppBar(title = { Text(client.name) }) }) { padding ->
        LazyColumn(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                Info("CURP", client.curp); Info("RFC", client.rfc); Info("NSS", client.nss); Info("Teléfono", client.phone)
                Info("Correo", client.email); Info("AFORE", client.afore); Info("Observaciones", client.notes)
            } } }
            item { Text("Documentos", style = MaterialTheme.typography.titleLarge) }
            item { OutlinedTextField(pendingLabel, { pendingLabel = it }, Modifier.fillMaxWidth(), label = { Text("Tipo o nombre del documento") }, singleLine = true) }
            ...
            items(client.documents, key = { it.id }) { doc ->
                Card(Modifier.fillMaxWidth()) { Row(Modifier.padding(12.dp).fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Column(Modifier.weight(1f)) { Text(doc.label, style = MaterialTheme.typography.titleMedium); Text(doc.mimeType.ifBlank { "Archivo" }, style = MaterialTheme.typography.bodySmall) }
                    TextButton(onClick = { runCatching { context.startActivity(Intent(Intent.ACTION_VIEW).apply { setDataAndType(Uri.parse(doc.uri), doc.mimeType.ifBlank { "*/*" }); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }) } }) { Text("Abrir") }
    ...
    if (showDelete) AlertDialog(onDismissRequest = { showDelete = false }, title = { Text("Eliminar expediente") },
        text = { Text("Se eliminarán los datos guardados de ${client.name}. Los archivos originales del teléfono no se borrarán.") },
        confirmButton = { TextButton(onClick = onDelete) { Text("Eliminar") } }, dismissButton = { TextButton(onClick = { showDelete = false }) { Text("Cancelar") } })
}

@Composable
private fun Info(label: String, value: String) { if (value.isNotBlank()) Text("$label: $value") }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Portal web${client?.name?.let { " · $it" } ?: ""}") }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding)) {
            Row(Modifier.fillMaxWidth().padding(8.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedTextField(url, { url = it }, Modifier.weight(1f), label = { Text("Dirección HTTPS del portal") }, singleLine = true)
                Button(onClick = { val t = url.trim(); loadedUrl = when { t.startsWith("https://") -> t; t.startsWith("http://") -> t.replaceFirst("http://", "https://"); t.isNotBlank() -> "https://$t"; else -> null } }) { Text("Ir") }
            }
            Text("La selección de archivos funciona cuando el portal ofrece Subir archivo. La verificación facial conserva la cámara real.", Modifier.padding(horizontal = 12.dp, vertical = 4.dp), style = MaterialTheme.typography.bodySmall)
            loadedUrl?.let { address -> AndroidView(Modifier.weight(1f).fillMaxWidth(), factory = { context -> WebView(context).apply {
                ...
                webChromeClient = object : WebChromeClient() {
                    override fun onShowFileChooser(webView: WebView?, callback: ValueCallback<Array<Uri>>?, params: FileChooserParams?): Boolean {
                        ...
                        return try { chooser.launch(params?.createIntent() ?: Intent(Intent.ACTION_OPEN_DOCUMENT).apply { addCategory(Intent.CATEGORY_OPENABLE); type = "*/*" }); true }
                        catch (_: Exception) { fileCallback = null; false }
                    }
                }; loadUrl(address)
            } }, update = { if (it.url != address) it.loadUrl(address) }) } ?: Card(Modifier.padding(16.dp).fillMaxWidth()) { Text("Escribe la dirección oficial del portal para abrirlo de forma segura.", Modifier.padding(16.dp)) }
...
private object ClientStore {
    fun load(context: Context): List<Client> = runCatching {
        val array = JSONArray(context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString(CLIENTS_KEY, "[]") ?: "[]")
        buildList { for (i in 0 until array.length()) add(clientFromJson(array.getJSONObject(i))) }
    }.getOrDefault(emptyList())
    fun save(context: Context, clients: List<Client>) {
        val array = JSONArray(); clients.forEach { array.put(clientToJson(it)) }
    ...
    private fun clientToJson(c: Client) = JSONObject().apply {
        put("id", c.id); put("name", c.name); put("curp", c.curp); put("rfc", c.rfc); put("nss", c.nss); put("phone", c.phone)
        put("email", c.email); put("afore", c.afore); put("notes", c.notes); put("documents", JSONArray().apply { c.documents.forEach { d -> put(JSONObject().apply { put("id", d.id); put("label", d.label); put("uri", d.uri); put("mimeType", d.mimeType) }) } })
    }
    private fun clientFromJson(o: JSONObject): Client {
        val docs = o.optJSONArray("documents") ?: JSONArray()
        return Client(id = o.optString("id", UUID.randomUUID().toString()), name = o.optString("name"), curp = o.optString("curp"), rfc = o.optString("rfc"), nss = o.optString("nss"), phone = o.optString("phone"), email = o.optString("email"), afore = o.optString("afore"), notes = o.optString("notes"), documents = buildList {
            for (i in 0 until docs.length()) docs.optJSONObject(i)?.let { d -> add(ClientDocument(id = d.optString("id", UUID.randomUUID().toString()), label = d.optString("label", "Documento"), uri = d.optString("uri"), mimeType = d.optString("mimeType"))) }
        })
    }
}
