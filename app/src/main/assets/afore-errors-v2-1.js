(() => {
  if (window.__dvAforeErrorsV21) return;
  window.__dvAforeErrorsV21 = true;
  ...
  const errors = [
    ...
    ['E28','Cámara','AforeWeb','No se pudo acceder a la cámara.','Permiso bloqueado o cámara ocupada.','Autorizar cámara, cerrar otras apps y recargar.','Alta'],
    ...
    ['MC 007','En investigación','AforeMóvil','Mensaje completo pendiente de confirmar.','Evidencia pública insuficiente.','Conservar captura y reportar etapa exacta del trámite.','Baja'],
    ['MC 521','En investigación','AforeMóvil','Mensaje completo pendiente de confirmar.','Evidencia pública insuficiente.','Conservar captura y reportar etapa exacta del trámite.','Baja']
  ].map((e, i) => ({id:i+1, code:e[0], category:e[1], app:e[2], message:e[3], cause:e[4], solution:e[5], confidence:e[6]}));

  const style = document.createElement('style');
  style.textContent = `
    #dvAforeErrorsView{display:none;max-width:1050px;margin:auto;padding:15px}
    #dvAforeErrorsView.active{display:block}
    #dvAforeErrorsView .error-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:12px;margin-top:14px}
    #dvAforeErrorsView .error-card{background:var(--card);border:1px solid var(--line);border-radius:15px;padding:14px}
    #dvAforeErrorsView .error-code{font-size:1.12rem;font-weight:900;color:var(--primary)}
    #dvAforeErrorsView .error-meta{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}
    #dvAforeErrorsView .error-chip{border:1px solid var(--line);border-radius:999px;padding:4px 8px;font-size:.75rem;font-weight:700}
    #dvAforeErrorsView .confidence-Alta{color:#087f5b}.confidence-Media{color:#a15c00}.confidence-Baja{color:#b42318}
    #dvAforeSearch{width:100%;padding:12px;border:1px solid var(--line);border-radius:10px;background:var(--card);color:var(--text)}
    #dvAforeCategory{width:100%;padding:11px;border:1px solid var(--line);border-radius:10px;background:var(--card);color:var(--text);margin-top:8px}
    #dvAforeFallbackBtn{position:fixed;left:12px;top:72px;z-index:55;width:48px;height:48px;border-radius:14px;padding:0;font-size:23px;box-shadow:0 4px 14px #0003}
    .dv-afore-menu-item{width:100%;text-align:left;margin:4px 0}
  `;
  ...
  view.id = 'dvAforeErrorsView';
  view.innerHTML = `
    <div class="card">
      <div class="hero"><div><h1>⚠️ Errores AFORE</h1><div class="small">Catálogo inicial de ${errors.length} códigos y mensajes documentados</div></div><button id="dvAforeBack" class="secondary">← Volver</button></div>
      <input id="dvAforeSearch" placeholder="🔎 Buscar código, mensaje, causa o solución">
      <select id="dvAforeCategory"><option value="">Todas las categorías</option></select>
      <div id="dvAforeCount" class="small" style="margin-top:9px"></div>
    </div>
    <div id="dvAforeResults" class="error-grid"></div>`;
  app.after(view);

  const categories = [...new Set(errors.map(e => e.category))].sort();
  const select = view.querySelector('#dvAforeCategory');
  categories.forEach(c => { const o=document.createElement('option'); o.value=c; o.textContent=c; select.appendChild(o); });

  function esc(v){return String(v ?? '').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
  function renderErrors(){
    const q = view.querySelector('#dvAforeSearch').value.trim().toLowerCase();
    const cat = select.value;
    const filtered = errors.filter(e => (!cat || e.category===cat) && (!q || Object.values(e).join(' ').toLowerCase().includes(q))));
    view.querySelector('#dvAforeCount').textContent = `${filtered.length} resultado${filtered.length===1?'':'s'}`;
    view.querySelector('#dvAforeResults').innerHTML = filtered.map(e => `<article class="error-card"><div class="error-code">${esc(e.code)}</div><div class="error-meta"><span class="error-chip">${esc(e.app)}</span><span class="error-chip">${esc(e.category)}</span><span class="error-chip confidence-${esc(e.confidence)}">Confianza ${esc(e.confidence)}</span></div><p><strong>Mensaje:</strong> ${esc(e.message)}</p><p><strong>Causa probable:</strong> ${esc(e.cause)}</p><p><strong>Qué hacer:</strong> ${esc(e.solution)}</p></article>`).join('') || '<div class="card"><p>No se encontraron errores con esos criterios.</p></div>';
  }

  function openErrors(){ app.style.display='none'; view.classList.add('active'); window.scrollTo(0,0); renderErrors(); }
  function closeErrors(){ view.classList.remove('active'); app.style.display=''; window.scrollTo(0,0); }
  view.querySelector('#dvAforeSearch').addEventListener('input', renderErrors);
  select.addEventListener('change', renderErrors);
  view.querySelector('#dvAforeBack').addEventListener('click', closeErrors);

  function installMenuItem(){
    const candidates = [...document.querySelectorAll('nav,aside,[role="navigation"],.drawer,.side-menu,.sidebar,#sideMenu,#drawerMenu')];
    const menu = candidates.find(x => x.offsetParent !== null || x.children.length);
    if (menu && !menu.querySelector('.dv-afore-menu-item')) {
      const b=document.createElement('button'); b.type='button'; b.className='secondary dv-afore-menu-item'; b.textContent='⚠️ Errores AFORE'; b.addEventListener('click', openErrors); menu.appendChild(b); return true;
    }
    return false;
  }
  if (!installMenuItem()) {
    const b=document.createElement('button'); b.id='dvAforeFallbackBtn'; b.type='button'; b.textContent='⚠️'; b.title='Errores AFORE'; b.setAttribute('aria-label','Abrir Errores AFORE'); b.addEventListener('click',openErrors); document.body.appendChild(b);
  }
